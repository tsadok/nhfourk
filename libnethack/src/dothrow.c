...
/* Last modified by Jonadab the Unsightly One, 2017-Sep-26 */
        ...
        case EGG:
            if (uarmh
                ...
                pline(msgc_noconsequence,
                      "You've got it all over your visor!");
                break;
            } else{
                /* egg ends up "all over your face" */
                pline(msgc_consequence, "You've got it all over your %s!",
                      body_part(FACE));
                if (petrifier && !Stone_resistance &&
                    !(poly_when_stoned(youmonst.data) &&
                      polymon(PM_STONE_GOLEM, FALSE))) {
                    if (uarmh)
                        pline(msgc_fatal, "Your %s fails to protect you.",
                              xname(uarmh));
                    goto petrify;
                }
            }
